...
export PATH=$PATH:/snap/bin && lxc image import /root/image/debian11.tar.gz --alias debian11
        ;;
        3)
        ...
        ;;
        4)
            if [ ! -f /root/image/hwspanel.tar.gz ]; then
                curl -L http://system.senkin.nl/hwspanel.tar.gz -o /root/image/hwspanel.tar.gz
                echo "下载完成，开始导入镜像"
            fi
            cd /root >/dev/null 2>&1
            lxc image import /root/image/hwspanel.tar.gz --alias hwspanel
        ;;
    esac
}
...
main(){
    echo " ____             _    _         _              "
    echo "/ ___|  ___ _ __ | | _(_)_ __   | |   __  _____ "
    echo "\___ \ / _ \ '_ \| |/ / | '_ \  | |   \ \/ / __|"
    echo " ___) |  __/ | | |   <| | | | | | |___ >  < (__ "
    echo "|____/ \___|_| |_|_|\_\_|_| |_| |_____/_/\_\___|"
    echo "                                                "
    echo " ___           _        _ _                       "
